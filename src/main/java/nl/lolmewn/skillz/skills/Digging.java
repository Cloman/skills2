...
public class Digging extends Skill {
    ...
    private void handleDoubleDrop(SkillzPlayer player, BlockBreakEvent event) {
        ...
        if (r.nextDouble() * 100
                < MathProcessor.processEquation(this.getMessage("special.doubleDrops.chance", "%level%/3").replace("%level%", "" + player.getLevel(this)).replace("%xp%", "" + player.getXP(this)))) {
    ...
    private void handleDoubleXp(SkillzPlayer player, BlockBreakEvent event) {
        ...
        if (r.nextDouble() * 100
                < MathProcessor.processEquation(this.getMessage("special.doubleXp.chance", "%level%").replace("%level%", "" + player.getLevel(this)).replace("%xp%", "" + player.getXP(this)))) {
            if (this.getSkillConfig().getBoolean("special.doubleDrops.notifyPlayer", true)) {
                event.getPlayer().sendMessage(
                        ChatColor.translateAlternateColorCodes('&',
